...
먼저 흔히 오해하기 쉬운 점을 짚고 가겠습니다. 매매 의사결정은 AI가 아니라 **코드로 정의된 알고리즘** — 구체적으로
기술적 분석 신호, 포트폴리오 최적화, 리스크 규칙 — 이 내립니다. 즉 AI가 실시간으로 종목을 고르거나 주문을
던지는 것이 아닙니다. AI 에이전트의 역할은 비전문가가 그 알고리즘 프로그램을 **개발·연구·리뷰**하도록 돕는
것입니다. 그래서 이 시리즈에서 "AI 팀"은 코드를 만드는 개발 조직을 뜻하고, "제안(proposal)"은 LLM이 즉흥적으로
고른 것이 아니라 알고리즘 서비스(quant-research 등)가 코드로 계산해 낸 결과를 뜻합니다.
...
실행(execution) 서비스는 **fail-closed**(의심스러우면 일단 막는다) 제1원칙을 따릅니다. 즉, risk-engine의 HMAC 토큰 없이는 주문이 제출되지 않습니다.
