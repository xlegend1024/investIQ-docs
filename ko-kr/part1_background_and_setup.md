...
매매 의사결정은 **코드로 정의된 알고리즘** — 기술적 분석 신호, 포트폴리오 최적화, 리스크 규칙 — 이 내립니다.
...
택한 접근은 AI 에이전트로 개발·연구 조직을 구성하는 것이었습니다. 내부적으로 "Squad"라 부르는 이 팀은 사람
...
InvestIQ는 미국 주식 스윙 트레이딩을 **승인 게이트(approval-gated)** 모델로 자동화합니다: 분석은 자동,
실행은 사람 승인.
...
실행 서비스는 **fail-closed** 제1원칙을 따릅니다: risk-engine의 HMAC 토큰 없이는 주문이 제출되지 않습니다.
