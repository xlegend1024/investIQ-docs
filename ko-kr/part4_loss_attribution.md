...
이 기간의 결과는 하나의 숫자로 요약됩니다. 평가금 기준으로 계정 자기자본이 약 −$878 감소했습니다 (≈$25,000 → ≈$24,122). 
아래 종목별 분석은 이 변화를 분해한 한 조각 위에 구축됩니다. 실험 기간동안 종목별로
매수를 매도에 FIFO 방식으로 매칭하여 얻은 **롱 전용 마감 라운드트립 실현손익 −$369.85**입니다. 이 값은 마감된
롱 거래만을 측정한 것으로, 자기자본 변화 −$878의 일부일 뿐이며 나머지는 아래 세 구성요소에서 나옵니다.
...
참고로 이 시스템은 롱(매수) 전용으로 설계되어 숏을 의도적으로 매매하지 않습니다. 위 22건의 `sell_short`는 숏
전략이 아니라 매도 주문 수량이 그 시점의 보유분을 초과해 우발적으로 열린 소규모 잔여 숏이며, 자기자본 정산을
정확히 하기 위해 포함하였을 뿐입니다.

이 편의 나머지 종목별 분석은 깨끗하게 매칭된 롱 기록(−$369.85)을 다룹니다. 잔여 숏과 미청산 포지션이 그 위에
...
별도 연구는 뉴스 감성 신호가 거래 종목들의 미래 수익을 예측했는지 물었습니다. 이 연구의
...
핵심은 단순합니다. 분산되고 손익분기에 가까운 포트폴리오가 단일 종목에 기간을 내주었고, 해법은 그 단일
종목을 측하는 것이지 전략을 재구축하는 것이 아닙니다.
