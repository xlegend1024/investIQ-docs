...
뉴스 연구(4편)는 데이터를 보기 전에 가설·검정·합격 기준을 문서로 고정했습니다(사전등록).
...
제안 → 실행 전 과정이 자동인 시스템에서, 거부권은 사람의 클릭이 아니라 **코드**에 있어야 합니다. HMAC 토큰,
fail-closed 기본값, 종목별 하드 캡이 돈이 움직이는 그 한 단계에서 거부권을 행사합니다. 사람은 개별 주문이
아니라 그 위의 스위치 — 킬 스위치와 실자본(live) 무장 — 만 통제합니다. 완전 자동화가 위험한 게 아니라,
**거부권 없는 자동화**가 위험합니다.
...
| 실행 | 돈이 움직이는 단계에 거부권이 있는가? | 거부권(코드) 없이 돈이 움직이는 자동화 |
